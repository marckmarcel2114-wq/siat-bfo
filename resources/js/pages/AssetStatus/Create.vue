...
import { ArrowLeft, Save, Activity } from 'lucide-vue-next';
...
const submit = () => {
    form.post(route('asset-status.store'));
};

const breadcrumbs = [
    { title: 'Inicio', href: route('dashboard') },
    { title: 'Configuración', href: '#' },
    { title: 'Estados de Activo', href: route('asset-status.index') },
    { title: 'Nuevo', href: '#' },
];
</script>
        ...
        <div class="max-w-2xl mx-auto p-4 md:p-8">
            <div class="mb-8">
                <Button variant="ghost" as-child class="-ml-2 mb-4 text-muted-foreground hover:text-foreground">
                    <Link :href="route('asset-status.index')">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-extrabold tracking-tight text-foreground">Nuevo Estado</h1>
                        <p class="text-muted-foreground">Registre un nuevo estado de ciclo de vida.</p>
                    </div>
                </div>
            </div>
            ...
            <Card class="border-border/60 shadow-xl shadow-primary/5 bg-card/50 backdrop-blur-sm overflow-hidden">
                <form @submit.prevent="submit">
                    <CardHeader class="border-b border-border/50 bg-muted/20 pb-6">
                        <CardTitle class="text-xl text-primary/80">Información del Estado</CardTitle>
                    </CardHeader>
                    
                    <CardContent class="p-6 space-y-6">
                        <div class="space-y-2">
                            <Label for="nombre">Nombre del Estado <span class="text-destructive">*</span></Label>
                            <Input 
                                id="nombre" 
                                v-model="form.nombre" 
                                required 
                                class="h-11 bg-background/50 focus-visible:ring-primary shadow-sm"
                                placeholder="Ej. Nuevo, En Uso, Dañado..."
                            />
                    ...
                    <CardFooter class="flex justify-end gap-3 border-t border-border/50 bg-muted/10 px-6 py-4">
                        <Button variant="ghost" type="button" as-child class="hover:bg-background">
                            <Link :href="route('asset-status.index')">Cancelar</Link>
                        </Button>
                        <Button 
                            type="submit" 
                        ...
                        >
                            ...
                            {{ form.processing ? 'Guardando...' : 'Guardar Estado' }}
                        </Button>
                    </CardFooter>
                </form>
            </Card>
