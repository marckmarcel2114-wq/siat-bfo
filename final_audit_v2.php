...
$output = "Total ATMs: " . Ubicacion::where('tipo_sucursal_id', $atmId)->count() . "\n";
foreach (City::all() as $c) {
    ...
    if ($count > 0 || $branches > 0) {
        $output .= "- {$c->nombre}: $count ATMs, $branches Branches\n";
    }
}
file_put_contents('final_audit_full.txt', $output);
echo "Done\n";
